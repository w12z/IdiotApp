...
Sub SendImage()                             '发送图片过程
    Dim Temp() As Byte
    bSend = False
    CaptureScreen App.Path + "\TempImage.jpg"
    Do
    Loop Until Dir(App.Path + "\TempImage.jpg")
    SC "SIZ " & FileLen(App.Path + "\TempImage.jpg")
    Open App.Path + "\TempImage.jpg" For Binary As #1
        ReDim Temp(LOF(1) - 1)
        Get #1, , Temp
    Close #1
    Me.wsImage.SendData Temp
End Sub
...
Private Sub wsCommand_ConnectionRequest(ByVal requestID As Long)
    Me.wsCommand.Close
    Me.wsCommand.Accept requestID
    Me.labState.Caption = "状态:Connected."
    SC "CNT"
End Sub
...
Private Sub wsImage_ConnectionRequest(ByVal requestID As Long)
    Me.wsImage.Close
    Me.wsImage.Accept requestID
    Me.labState.Caption = "状态:Connected."
End Sub
